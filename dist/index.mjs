...
function isClassValidatorAvailable2() {
  try {
    __require("class-validator");
    return true;
  } catch (e) {
...
function isClassValidatorAvailable3() {
  try {
    __require("class-validator");
    return true;
  } catch (e) {
...
function inheritTransformationMetadata(parentClass, targetClass, isPropertyInherited, stackDecorators = true) {
  if (!isClassTransformerAvailable2()) {
    return;
  }
  try {
...
function isClassTransformerAvailable2() {
  try {
    __require("class-transformer");
    return true;
  } catch (e) {
...
function isClassValidatorAvailable4() {
  try {
    __require("class-validator");
    return true;
  } catch (e) {
...
// src/helpers/omit-type.helper.ts
...
// src/helpers/partial-type.helper.ts
...
// src/helpers/pick-type.helper.ts
...
export { IntersectionType, OmitType, PartialType, PickType, applyIsOptionalDecorator, applyValidateIfDefinedDecorator, inheritPropertyInitializers, inheritTransformationMetadata, inheritValidationMetadata, isClassTransformerAvailable, isClassValidatorAvailable, logger };
